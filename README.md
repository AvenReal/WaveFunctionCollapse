...
the `void debug_collapse(field_t *field, char *displayer[field->tt_nb_of_classes], int steps)` function. (see example3 for a demo)_
...
_Tips: since the `rand()` function output the same number each time a c program runs, the result of two separate runs of the same code will output the same grid, to change that, you can use the `srand()` function. (see example2 for a demo)_
